...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func dump() {
        ...
        for s in snap.sessions {
            print("session[\(s.shortId)] \(s.tool.display) \(s.label) \(Int(s.usedPercent))% \(s.ctxTokens)/\(s.windowSize) \(s.model) title_from=\(s.titleSource) cache_ttl=\(Int(s.cacheTTL)) cache_left=\(s.cacheLeft.map { String(Int($0)) } ?? "-") last_ctx=\(s.ctxTokens) compactions=\(s.compactions) post=\(s.lastPostTokens) compact_at=\(s.measuredCompactionPoint.map { "\($0) measured" } ?? "\(s.compactionTokens(pct: 85)) from the setting") handover=\(s.handoverSaved.map(String.init) ?? "unknown") clear=\(s.needsClear(limit: limit)) idle=\(s.isIdle)")
        }

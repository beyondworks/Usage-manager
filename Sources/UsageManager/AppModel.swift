...
final class AppModel: ObservableObject {
    ...
    func evaluateAlerts() {
        ...
        for s in sessions where s.hasContext && s.wantsCompactionAlert {
            ...
            if s.ctxTokens >= arm, st.armed, alertsOn {
                ...
                Gate.recordWarning(sessionId: s.sessionId, tokens: s.ctxTokens)
    ...
    func loadDemoData() {
        ...
        // The list behaves differently once it runs past five rows, and the gauge behaves
        // differently at high occupancy, so a snapshot can ask for a given number of rows
        // or for given percentages.
        let env = ProcessInfo.processInfo.environment
        let asked = env["USAGE_MANAGER_DEMO_PCT"]?.split(separator: ",").compactMap { Int($0) }
        if let want = asked?.count ?? env["USAGE_MANAGER_DEMO_ROWS"].flatMap(Int.init) {
            ...
            sessions = (0..<max(1, want)).map { i in
                SessionCtx(tool: .claudeCode, sessionId: "demo\(i)", project: "proj\(i)",
                           ...
                           ctxTokens: asked.map { $0[i] * 10_000 } ?? (880_000 - i * 95_000),

...
- CPU **1% 안팎** (3분씩 네 번 실측 0.28 / 0.78 / 1.19 / 1.43% — 추적 중인 세션이 바쁠수록 올라간다), 메모리 **30MB대**
- 수치는 `phys_footprint` 기준이다. 활성 모니터가 보여 주는 값이자 macOS가 앱 부담으로 치는 값이며, 상주 크기(RSS)보다 크다. `--mem` 으로 단계별로 찍어 볼 수 있다
- 실측(팝오버를 열지 않은 상주 상태): 시작 직후 14MB, 전체 스캔을 마치고 32MB, 그 뒤 30초 동안 34MB. 전체 스캔이 가장 많이 쓰는 순간의 최고치가 41MB다
- 데스크톱 앱의 세션 메타데이터는 이 중 2MB만 차지한다. 파일 하나가 500KB에 이르고 수백 개지만, 필요한 건 네 필드뿐이라 통째로 해석하지 않고 바이트에서 뽑아내며 파일은 복사 대신 매핑해서 본다
- 나머지는 대화 기록을 읽는 몫이다. 압축 횟수를 세려면 기록 전체를 한 번은 읽어야 하는데(가장 큰 것이 47MB다) 그 읽기를 한 덩어리씩 풀어 주지 않아 스캔이 끝날 때까지 모두 남아 있었다. 그 한 가지가 95MB와 30MB대를 가른다

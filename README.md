...
압축이 막 끝난 직후에는 아직 새 응답이 없어 읽을 토큰 수가 없다. 그때는 압축이 남긴 크기를 쓴다. 경계 너머까지 거슬러 읽으면 압축 전 크기가 잡혀서, 방금 비워진 세션을 곧 가득 찬다고 알리게 된다.
...
Claude 한도는 Claude Code가 최신으로 유지하는 keychain 항목을 읽어 조회한다. 유효한 동안(최대 한 시간) 메모리에 두고 거절당하면 그 자리에서 버리므로, 조회할 때마다 keychain을 다시 열지 않는다. **다른 계정으로 로그인하면 그 항목이 다시 쓰이므로, 항목의 수정 시각을 함께 보고 바뀌었으면 들고 있던 것을 버린다** — 그러지 않으면 한 시간 가까이 전 계정의 한도를 보여 준다. 수정 시각만 묻는 질의라 자격 값을 돌려받지 않고 승인도 필요 없다.

**보이는 것은 Claude Code가 로그인한 계정의 한도다.** 데스크톱 앱에서 다른 계정으로 바꿨다면 `claude auth login` 으로 같은 계정을 맞춰 줘야 한다. 데스크톱이 쓰는 계정의 한도를 읽을 로컬 경로는 없다 — 그 자격은 앱 안에만 있고, 세션 메타데이터에도 statusline 에도 한도가 실리지 않는다. 조회가 막히거나 앱을 막 켠 참이면 마지막으로 성공한 조회값을 흐리게 보여 준다. 그 값은 어느 계정 것인지와 함께 저장해 두고 지금 로그인한 계정 것일 때만 쓰므로, 다른 계정으로 떠 있는 터미널 세션의 값이 섞이지 않는다. 읽을 때마다 로그에 한 줄이 남는데 남는 것은 지문뿐이고 값은 기록하지 않는다. 요청은 `claude-code/<설치된 버전>`으로 자신을 밝힌다. 이 엔드포인트가 낯선 호출자에게 더 엄한 제한을 걸기 때문이다.
...
.build/debug/UsageManager --cache-check  # 자격 캐시 규칙(계정 전환 시 버리는지)
```

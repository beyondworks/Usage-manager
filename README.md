...
**보이는 것은 Claude Code가 로그인한 계정의 한도다.** 데스크톱 앱에서 다른 계정으로 바꿨다면 `claude auth login` 으로 같은 계정을 맞춰 줘야 한다. 데스크톱이 쓰는 계정의 한도를 읽을 로컬 경로는 없다 — 그 자격은 앱 안에만 있고, 세션 메타데이터에도 statusline 에도 한도가 실리지 않는다. 읽을 때마다 로그에 한 줄이 남는데 남는 것은 지문뿐이고 값은 기록하지 않는다. 요청은 `claude-code/<설치된 버전>`으로 자신을 밝힌다. 이 엔드포인트가 낯선 호출자에게 더 엄한 제한을 걸기 때문이다.
